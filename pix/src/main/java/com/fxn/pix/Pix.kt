...
public class Pix extends AppCompatActivity implements View.OnTouchListener {
    ...
    private int cameraFlashMode = ImageCapture.FLASH_MODE_OFF;
    ...
    private View status_bar_bg, mScrollbar, topbar, bottomButtons, topButton, sendButton;
    private TextView mBubbleView, img_count;
    private ImageView mHandleView, selection_back, selection_check, btnBack;
    ...
    public static void start(final Fragment context, final Options options) {
        PermUtil.checkForCamaraWritePermissions(context, options.getMode(), check -> {
    ...
    public static void open(final FragmentActivity context, final Options options, final ActivityResultLauncher<Intent> launcher) {
        PermUtil.checkForCamaraWritePermissions(context, options.getMode(), check -> {
            Intent i = new Intent(context, Pix.class);
            i.putExtra(OPTIONS, options);
            launcher.launch(i);
    ...
    public static void open(final FragmentActivity context, final Options.Mode type, final ActivityResultLauncher<Intent> launcher) {
    ...
    private void initialize() {
        ...
        statusBarHeight = Utility.getStatusBarSizePort(Pix.this);
        ...
        if (dirs.length > 0) {
            File mdir = new File(dirs[0], "Pix");
            mdir.mkdirs();
            if (mdir != null && mdir.exists()) {
                outputDirectory = mdir;
            } else {
        ...
        status_bar_bg.getLayoutParams().height = statusBarHeight;
        status_bar_bg.setTranslationY(-1 * statusBarHeight);
        ...
        FrameLayout.LayoutParams lp3 = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.WRAP_CONTENT);
        lp3.setMargins(0, statusBarHeight, 0, 0);
        topButton.setLayoutParams(lp3);

        FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) sendButton.getLayoutParams();
        layoutParams.setMargins(0, 0, (int) (Utility.convertDpToPixel(16, this)), (int) (Utility.convertDpToPixel(174, this)));
    ...
    private void takePhoto() {
        ...
        // been taken
        imageCapture2.takePicture(outputOptions, ContextCompat.getMainExecutor(this), new ImageCapture.OnImageSavedCallback() {
            @Override
            public void onImageSaved(@NonNull @NotNull ImageCapture.OutputFileResults outputFileResults) {
                runOnUiThread(() -> {
                    ...
                    Utility.vibe(Pix.this, 50);
                    Img img = new Img("", "", photoFile.getAbsolutePath(), "", 1);
                    selectionList.add(img);
                    //Log.e("result photo", "->" + photo.getAbsolutePath());
            ...
            public void onError(@NonNull @NotNull ImageCaptureException exception) {
                Toast.makeText(Pix.this, String.format(getResources().getString(R.string.pix_failed_take_photo), "" + exception.getLocalizedMessage()), Toast.LENGTH_LONG).show();
            }
        });
    }
...
//            final CameraControl cameraControl = camera.getCameraControl();
    ...
    private void onClickMethods() {
        ...
        clickme.setOnClickListener(view -> {
            if (selectionList.size() >= options.getCount()) {
                Toast.makeText(Pix.this, String.format(getResources().getString(R.string.cannot_click_image_pix), "" + options.getCount()), Toast.LENGTH_LONG).show();
                return;
            }
            ...
            final ObjectAnimator oj = ObjectAnimator.ofFloat(previewView, "alpha", 1f, 0f, 0f, 1f);
    ...
    private void updateImages() {
        ...
        Cursor cursor = Utility.getImageVideoCursor(Pix.this, true);
        if (cursor == null) {
            return;
        }
        ...
        imageVideoFetcher.execute(Utility.getImageVideoCursor(Pix.this, true)); //options.getMode()
        cursor.close();
    ...
    private void setBottomSheetBehavior() {
        ...
        mBottomSheetBehavior.addBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() {
            ...
            public void onSlide(@NonNull @NotNull View bottomSheet, float slideOffset) {
                if (options.getType() == Options.Mode.Both) {
                ...
                if (slideOffset == 1) {
                    ...
                    hideScrollbar();
                    img_count.setText(String.valueOf(selectionList.size()));
                    if (options.getType() == Options.Mode.Gallery) {
                        finish();
                    }
                    //--camera.open();
                }

 ...
 */
class HeaderItemDecoration(private val context: Context, private val mListener: StickyHeaderInterface) : RecyclerView.ItemDecoration() {
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        super.onDrawOver(c, parent, state)
        val topChild = parent.getChildAt(0)
        if (Utility.Companion.isNull(topChild)) {
            return
        }
        ...
        currentHeader.setPadding(
            (currentHeader.paddingLeft - Utility.Companion.convertPixelsToDp(5f, context)).toInt(),
        ...
        if (Utility.Companion.isNull(childInContact)) {
